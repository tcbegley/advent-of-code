...
def process_pulses(pulses, network, part_2=False):
    low_pulse_count = high_pulse_count = 0

    while pulses:
        source, module_name, pulse = pulses.popleft()
        if part_2 and module_name == "rx" and pulse == 0:
            return None
        if pulse == 1:
            high_pulse_count += 1
        else:
...
def part_2(network):
    button_presses = 0
    while True:
        print([network.modules[module] for module in network.incoming["ck"]])
        pulses = deque([(None, "broadcaster", 0)])
        if process_pulses(pulses, network) is None:
